...
function UploadReceiptPanel() : ReactElement {
    ...
    return (<>
        <div id="upload-receipt-div" className="absolute-center relative flex flex-col mr-5 items-baseline justify-center w-[25rem]">
            <a onClick={() => navigate('/receipts')} className="btn-nav absolute bg-red-400 top-1 right-1 m-5">
                <FontAwesomeIcon icon={faXmark} />
            </a>
            <h2 className="text-2xl font-semibold">Upload Receipt</h2>
            <label className="mt-2">Title</label>
            <input className="input" type="text" />
            <label className="mt-2">Used Card</label>
            <select className="select">
                <option value="">plc</option>
            </select>
            
            <label className="w-[100%] mt-2">Upload File</label>
