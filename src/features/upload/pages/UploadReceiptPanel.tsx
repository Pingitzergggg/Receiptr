...
import type { inputField } from "../../../shared/utils/types.ts";
import { stringValidate, type inputType } from "../../../shared/utils/stringValidator.ts";
import type { paginatable } from "../../../shared/utils/databaseTables.ts";
import {extractResponse, requestResource, type response, sendFileForm} from "../../../services/receiver.ts";
    ...
    useEffect(() => {
        if (currentFile) {
            if (currentFile[0].type === 'application/pdf') {
                setFileNotifier({
                    title: <p className="mt-[.5rem]">File: <b>{currentFile[0].name}</b></p>,
                ...
                setFileNotifier({
                    title: <p className="text-(--error-color) mt-[.5rem]">You must provide a <b>PDF</b> file!</p>,
        ...
        <div className="window upload w-[90%]! lg:w-[auto]!">
            ...
            <div className={updateReceipt ? "flex-col" : "grid grid-cols-1 md:grid-cols-2 gap-0"}>
                <div className='flex-col'>
                    ...
                    {!APIKey && <><Select onChange={(event) => cardId.current = Number(event.target.value)} errorInValue={false} id="card" title="Used Card" width="100%" values={cards} selected={cardId.current ?? ''} />
                ...
                <div className={updateReceipt ? '' :'flex-col md:ml-[2rem]'}>
                    ...
                    <Button async={true} onClick={upload} label="Upload" width="100%" fontSize="1rem" icon={<FontAwesomeIcon icon={faUpload} />}/>
                </div>
            </div>
        </div>
    </>);
